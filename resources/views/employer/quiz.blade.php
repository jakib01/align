...
@section('quiz')
        ...
        <section class="section">
            <div class="container">
                ...
                <div class="pagination-controls">
                    ...
                    @if ($page > 1)
                        <a href="{{ url('employer/value/assessment/' . ($page - 1)) . "/" .$token }}" class="btn btn-secondary">
                            Previous
                        </a>
                    @endif

                    @if ($page < $totalPages)
                        <a href="{{ url('employer/value/assessment/' . ($page + 1)) . "/" .$token }}" class="btn btn-primary" onclick="saveToLocalStorage({{ $page }})">
                            Next
                        </a>
                    @else
    ...
    <script>
        ...
        function finalSubmit() {
            fetch("{{ url('employer/value/assessment/submit') }}", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    token: "{{ $token }}" // Pass the server-side token here
                })            })
                .then((response) => {
                    if (!response.ok) {
